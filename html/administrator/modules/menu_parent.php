<?php
// Выводим меню и заисимость в компоненте при вызове на AJAX
...
{ 
	...
	$stmt_menu = $db->prepare("SELECT * FROM menu WHERE menu_type = :menu_type AND parent = :parent AND parent <> :menu_id AND id <> :menu_id ORDER BY ordering ASC");
	$stmt_menu->execute(array('menu_type' => $menu_type, 'parent' => $i, 'menu_id' => $menu_id));
