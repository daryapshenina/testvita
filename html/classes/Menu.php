...
class classMenu
{
	...
	private function leftTree($type, $view, $menu_id, $lvl, $menu_name, $menu_main, $menu_component, $p, $menu_prefix_css)
	{
		...
		$this->menu_out .= '<a class="'.$class.'-'.$lvl.$menu_prefix_css.'" href="'.$s.'">'.$menu_name.'</a>';
	...
	private function leftExtension($type, $view, $menu_id, $lvl, $menu_name, $menu_main, $menu_component, $p, $menu_prefix_css)
	{
		...
		else
		{
			...
			{
				$this->menu_out .= '<a class="'.$class.'-ext'.$menu_prefix_css.'" href="'.$s.'">'.$menu_name.'</a>';			
			}
		}
	...
	private function shopList($type, $view, $menu_id, $lvl, $menu_name, $menu_main, $menu_component, $p, $menu_prefix_css)
	{
		...
		if(count($child_arr_2) > 0) // если есть подпункты		
		{
			...
			{
				$this->menu_out .= '<div class="mod_menu_list_1_container"><a class="mod_menu_list_1" href="/'.$child_2_var_arr['url'].'">'.$child_2_var_arr['name'];
				...
				if(count($child_arr_3) > 0) // если есть подпункты		
				{
					...
					{
						$this->menu_out .= '<a href="/'.$child_3_var_arr['url'].'" class="mod_menu_list_2">'.$child_3_var_arr['name'].'</a>';
					}	
				}
		...
		{
			if ($lvl == 1) {$this->menu_out .= '<a class="'.$class.'-'.$lvl.$menu_prefix_css.'" href="'.$s.'">'.$menu_name.'</a>';}				
		}
