...
function account_image_resize($user_id)
{
	global $root;
	...
	if(!empty($file_name))
	{
		...
		$path = $root.'/files/account/'.$floor_id.'/'.$user_id;
		...
		if(!($ext == 'jpg' || $ext == 'jpeg' || $ext == 'gif' || $ext == 'png'))
		{
			...
			$err_log = '/components/account/admin/users/insert.php <br>Ресайз изображения.<br>Тип файла - не изображение. Мим тип <b>'.$file_name.'</b><br>Пользователь <b>'.Auth::check().'<b>';
		...
		{
			$err_text = 'Тип файла - не изображение формата jpg, gif, png';
			$err_log = '/components/account/admin/users/insert.php <br>Ресайз изображения.<br>Тип файла - не изображение. Мим тип <b>'.$type.'</b><br>Пользователь <b>'.Auth::check().'<b>';
			err($err_text, $err_log);
		}
		...
		$img_thumb = new ImageResizeSelectArea($file_tmp, $path.'/thumbnail.jpg', 200, 200);
		$img_thumb->setArea($x1, $y1, $w, $h);
		$img_thumb->run();

		$img_photo = new ImageResize($file_tmp, $path.'/photo.jpg', 800, 800);
		$img_photo->run();		
