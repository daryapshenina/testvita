...
function ads_image_resize($section_id)
{
	global $root;
	...
	if(!empty($file_name))
	{
		...
		if(!($ext == 'jpg' || $ext == 'jpeg' || $ext == 'gif' || $ext == 'png'))
		{
			...
			$err_log = '/components/ads/admin/section/insert.php <br>Ресайз изображения.<br>Тип файла - не изображение. Мим тип <b>'.$file_name.'</b><br>Пользователь <b>'.Auth::check().'<b>';
		...
		{
			$err_text = 'Тип файла - не изображение формата jpg, gif, png';
			$err_log = '/components/ads/admin/section/insert.php <br>Ресайз изображения.<br>Тип файла - не изображение. Мим тип <b>'.$type.'</b><br>Пользователь <b>'.Auth::check().'<b>';
			err($err_text, $err_log);
		}
		...
		$img_thumb = new ImageResizeSelectArea($file_tmp, $path.$section_id.'.jpg', 200, 200);
