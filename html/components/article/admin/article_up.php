...
$num = mysql_query("SELECT * FROM `menu` WHERE `id_com`='$id_com' AND `component`='article' AND `main`='1' LIMIT 1") or die ("Невозможно сделать выборку из таблицы - 1");
...
$num_parent = mysql_query("SELECT * FROM `menu` WHERE `parent` = '$menu_parent' ORDER BY `ordering` ASC") or die ("Невозможно сделать выборку из таблицы - 2");
...
// Обновляем данные в таблице "menu"
	$query_update_menu = "UPDATE `menu` SET `ordering` = '$row[ordering]' WHERE `id` = '$row[menuid]' AND `main` = '1';";
