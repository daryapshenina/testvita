...
function a_com()
{ 
	global $db, $domain, $d, $ordering, $id_com, $menu_type, $menu_top_selected, $menu_left_selected; 	
	
	// определяем содержимое меню	
	$stmt = $db->prepare("SELECT id, menu_type, name, parent, ordering FROM menu WHERE id_com = :id_com  AND component = 'page' LIMIT 1");
	$stmt->execute(array('id_com' => $id_com));
	...
	if($menu_type == 'top'){$menu_top_selected = 'selected';}
	if($menu_type == 'left'){$menu_left_selected = 'selected';}
