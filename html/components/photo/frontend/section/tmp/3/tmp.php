...
function component()
{
	...
	while($item = $stmt_item->fetch())
	{
		...
		if($item['link'] == '')
		{$a = '<a'.$edit_item.' href="/files/photo/'.$item['section'].'/'.$item['name'].'_.jpg" class="show photo_item">';}
		else {$a = '<a'.$edit_item.' href="'.$item['link'].'" class="photo_item">';}

		$item_out .= $a.
			'<img class="photo_img" src="/files/photo/'.$item['section'].'/'.$item['name'].'.jpg" alt="">'.
			'<div class="photo_item_content">'.
