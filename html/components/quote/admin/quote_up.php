...
if (!isset($tipmenu) || $menu_t == "0" || $menu_t== "1"){$menu_type = "top";}
if ($menu_t == "2"){$menu_type = "left";}
	
// Определяем родительский пункт меню
$num = mysql_query("SELECT * FROM `menu` WHERE `menu_type` = '$menu_type' AND `id_com`='$id_com' AND `component`='quote' AND `main`='1' LIMIT 1") or die ("Невозможно сделать выборку из таблицы - 1");
