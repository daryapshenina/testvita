...
// Наследуем класс Chars (он общий для frontend и admin)
class CharsAdmin extends Chars
{
	// Возвращает шаблон вывода характеристик
	protected function template($item_id)
	{
		$out = '';

		foreach($this->getArray($item_id) as $char) // цикл по строкам
		{
			if($char['type'] == 'number')
			{
				$type_out = 'число';
