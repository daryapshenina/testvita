...
class classShopItemCellFlat extends classShopItem
{
	protected function templateItem($_item)
	{
		...
		$photo_width = $this->shopSettings->getValue('x_small');
		$photo_height = $this->shopSettings->getValue('y_small');

		if($this->shopSettings->getValue('basket_type') == 1){$click = 'onclick="shop_buy_fly('.$id.');"';}
