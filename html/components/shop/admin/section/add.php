...
function a_com()
{ 
	global $db, $d, $domain, $section_id, $menu_type, $filter; 

	$stmt_select = $db->prepare("SELECT id, menu_type, parent, ordering FROM menu WHERE id_com = :section_id AND component = 'shop' AND (p1 = 'section' OR p1 = 'all') ORDER BY id LIMIT 1");
	...
	if(!isset($m['ordering'])){$m['ordering'] = 0;}

	$menu_top_selected = $menu_left_selected = '';
	
	if(isset($m['menu_type']) && $m['menu_type'] == 'top'){$menu_top_selected = 'selected';}
	else if(isset($m['menu_type']) && $m['menu_type'] == 'left'){$menu_left_selected = 'selected';}
	else{$menu_left_selected = 'selected';}
	...
	echo '
		<h1>ИНТЕРНЕТ - МАГАЗИН: Создать раздел</h1>
	
		<form method="POST" action="/admin/com/shop/section/insert">	
		...
		<table class="main_tab">
			...
			<tr>
				<td width="20">&nbsp;</td>		
				<td width="200" height="25">Название раздела <div class="help"><span class="tooltip"><img src="/administrator/tmp/images/question-50.png" alt="Помощь" /><em>Название раздела</em>Выводится над основным содержимым в теле страницы.<br><br> Для поисковых роботов система управления прописывает его специальным тегом заголовка - h1</span></div></td>
				<td><input type="text" name="title" size="50" required ></td>
			</tr>
			<tr>
				<td width="20">&nbsp;</td>			
				<td width="200" height="25">Опубликовать раздел <div class="help"><span class="tooltip"><img src="/administrator/tmp/images/question-50.png" alt="Помощь" /><em>Опубликовать страницу</em>Галочка стоит - раздел отображается. Нет галочки - раздел не отображается, пункт меню тоже не отображается.</span></div></td>
			...
			<tr>
				...
				<td>
					<select onChange="menu_type_select('.$m['id'].')" name="menu_type" id="menu_type">
		...
		<tr>
			...
			<td><input type="number" min="0" max="100000" name="menu_ordering" value="'.($m['ordering'] + 1).'" size="5"></td>
		</tr>			
			<tr>
				<td>&nbsp;</td>			
				<td height="25">&nbsp;</td>
		...
		<div id="leftaccordion" class="left_list seo_fon">	
			...
			<div class="left_body">                 
				<table>
					...
					<tr>
						<td>&nbsp;</td>			
						<td style="height:25px;">ЧПУ URL <div class="help"><span class="tooltip"><img src="/administrator/tmp/images/question-50.png" alt="Помощь" /><em>Человеко - понятный URL</em>Для каждой страницы можно прописать свой адрес вручную. Например, для раздела интернет-магазина <b>Косметика</b> можно указать такой адрес: <b>http://site.ru/cosmetics</b>, для страницы контакты: <b>http://site.ru/contacts</b><br><br>Если поле оставить пустым, система управления сгенерирует адрес в таком формате <b>http://site.ru/shop/section/777</b>, где 777 - номер раздела</span></div></td>
						<td>
