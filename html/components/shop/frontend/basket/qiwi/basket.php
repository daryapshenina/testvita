...
function component()
{
	global $root, $site, $dbname, $email, $ii, $kolich, $fio, $tel, $email_client, $address, $comments, $summa, $ip;
				...
				if ($id_b > 0)
				{
					...
					if ($item_result > 0)
					{
						while($b = mysql_fetch_array($item_query))
						{
							...
							$comments_qiwi .= $basket_item_title.'   '.$kolichestvo.' x '.$basket_item_price.' '.$shopSettings->currency.' = '.$sum.' '.$shopSettings->currency.'     ';

							$o = '<span>'.$basket_item_title.'&nbsp;&nbsp;&nbsp;&nbsp; '.$kolichestvo.' х '.$basket_item_price.' '.$shopSettings->currency.' = '.$sum.' '.$shopSettings->currency.'</span><br>' ;
	...
	$order = $order_middle.'<div><b>'.LANG_SUM.' '.$summa.' '.$shopSettings->currency.'</b></div>';
	...
	include($root."/components/shop/frontend/basket/tmp/tmp.php");
