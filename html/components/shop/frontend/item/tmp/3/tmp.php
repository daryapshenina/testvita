...
else
{
	...
	$tag_description = mb_substr($tag_description, 0, mb_strrpos(mb_substr($tag_description, 0, 400, 'utf-8'),' ','utf-8'),'utf-8'); // Обрезаем по пробелу.
}
...
function shop_item_tmp($item)
{
	...
	else
	{
		...
		$photo_big ='<img class="'.$show.' item_photo_big" style="max-width:'.$size[0].'px;" src="/components/shop/photo/'.$item['photo_big'].'" alt="'.$item['title'].'" />';
