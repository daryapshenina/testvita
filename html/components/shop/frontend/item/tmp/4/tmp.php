	...
	if(isset($item['char']))
	{
		foreach ($item['char'] as $name => $char)
		{
			if(count($char['value']) > 1)
			{
				...
				if(substr_count(mb_strtolower($name), 'цвет') > 0 || substr_count(mb_strtolower($name), 'color') > 0) // Если название характеристики содержит слово 'цвет' || 'color'
				{
					// перебираем характеристику
					foreach ($char['value'] as $option_value)
					{
						if($i == '0' && $option_value == $item['char'][$name]['value_item']){$color_selected = 'char_color_item_selected';}else{$color_selected = '';}

						if(is_file($root.'/files/shop/chars/colors/'.$option_value.'.jpg')){$img_src = '/files/shop/chars/colors/'.$option_value.'.jpg';}else{$img_src = '/components/shop/frontend/tmp/images/no-photo.png';}

						$char_color_item .= '<div id="char_color_'.$option_value.'" onclick="items_tmp_char_color(\''.$name.'\', \''.$option_value.'\');" class="char_color_item '.$color_selected.'"><img class="char_color_item_img" src="'.$img_src.'"><div class="char_color_item_name">'.$option_value.'</div></div>';
					}

					$char_color_out .= '<div class="char_color_container"><div class="char_color_title">'.$name.'</div>'.$char_color_item.'</div><input id="char_color_input" type="hidden" name="char['.$name.']" value="'.$item['char'][$name]['value_item'].'">';
				}
				else // Любая другая характеристика, не цвет
				{
					...
					foreach ($char['value'] as $option_value)
					{
						...
						if($i == '0' && $option_value == $item['char'][$name]['value_item']){$selected = 'selected';}else{$selected = '';}
			...
			if(substr_count(mb_strtolower($name), 'цвет') == 0 && substr_count(mb_strtolower($name), 'color') == 0) // Любая другая характеристика, не цвет
			{
				$item_char_tr .= '<tr>';
		...
		<form'.$edit_data.' class="'.$edit_class.'shop_item_container" method="POST" action="/shop/basket">
			<div itemscope itemtype="http://schema.org/Product">
				...
				<div id="item_main" class="item_main">
					...
					<div id="item_introtext_container">
						<div class="item_introtext_container_padding">
							'.$rating_out.'
							'.$item_char.'
							'.$char_color_out.'
							'.$intro_text_out.'
