...
function shop_item_tmp($item)
{
	...
	if ($shopSettings->question){$question_out = '<div onclick="DAN.modal.add(item_question, \'450\', \'0\')" class="but_cart_8 but_cart_8_question">'.LANG_SHOP_ITEM_ASK.'</div>';} else{{$question_out = '';}}
		...
		if($item['photo_more'] != '')
		{
			...
			if(count($photo_arr) >= 1)
			{
				for($i=0; $i<count($photo_arr)-1; $i++)
				{
					...
					$img_more_out .= '<a class="show" href="/components/shop/photo/'.$photo_arr_big[$i].'">
	...
	if(isset($item['char']))
	{
		foreach ($item['char'] as $name => $char)
		{
			...
			$item_char_tr .= '<tr>';
			$item_char_tr .= '<td class="item_char_td_1"><span>'.$name.$unit_out.'</span></td>';
			$item_char_tr .= '<td class="item_char_td_2"><span>'.$value_out.'</span></td>';
			$item_char_tr .= '</tr>';
				...
				<div id="item_main" class="item_main">
					<div id="item_photo_container">
						...
						<div class="item_price_main">
							'.$price_out.'
							<div class="shop_quantity_8"><div id="quantity_minus_8">-</div><input type="text" id="input_quantity_8" name="input_quantity" value="1" title="'.LANG_SHOP_ITEM_QUANTITY.'"><div id="quantity_plus_8">+</div></div>
							<div class="shop_but flex_row">
								<input type="submit" value="'.$shopSettings->sticker_add_to_cart.'" class="but_cart_8" name="shopbutton" />
								'.$question_out.'
							</div>
						</div>
					</div>
