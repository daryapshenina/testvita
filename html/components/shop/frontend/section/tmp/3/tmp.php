	...
	protected function templateItem($_item)
	{
		...
		<div '.$data_type.' class="'.$edit_mode_class.'section_item">
			<table class="section_item_ramka">
				<tr>
					<td colspan="2"><div><a class="shop-item-title-link" href="/shop/item/'.$_item['id'].'">'.$_item['title'].'</a></div></td>
				</tr>
				<tr>
					<td style="width:'.$shopSettings->x_small.'px; height:'.$shopSettings->y_small.'px; position:relative; text-align:center;">
						'.$new_out.$discount_out.$custom_out.$hit_out.'
						<div style="position:relative;">'.$photo_out.$photo_big_out.'</div>
						'.$rating_out.'
					...
					<td>
						<div style="margin-left:20px; text-align:left;">'.$_item['intro_text'].'</div>
					</td>
				</tr>
			</table>
		</div>
