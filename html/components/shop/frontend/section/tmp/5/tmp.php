<?php
// Плоской плиткой + всплывающие картинки
...
class classShopSectionItem extends classShopItem
{
	protected function templateItem($_item)
	{
		...
		if($_item['photo'] != '')
		{
			...
			{
				$photo_out = '<img border="0" alt="'.$_item['title'].'" src="/components/shop/photo/'.$_item['photo'].'" longdesc="/components/shop/photo/'.$_item['photo_big'].'" id="shop_item_img_'.$_item['id'].'" class="show2">';
			}
			else
			{
				$photo_out = '<img border="0" alt="'.$_item['title'].'" src="/components/shop/photo/'.$_item['photo'].'">';
			}
		}
		...
		<div '.$data_type.' class="'.$edit_mode_class.'section_item">
			...
			<div class="section_item_title" align="center" style="width:'.$shopSettings->x_small.'px;"><a href="/shop/item/'.$_item['id'].'"  title="'.LANG_SHOP_SECTION_READ_MORE.'">'.$_item['title'].'</a></div>
			<div style="width:'.$shopSettings->x_small.'px; height:'.$shopSettings->y_small.'px; position:relative;">
				<a href="/shop/item/'.$_item['id'].'" style="width:'.$shopSettings->x_small.'px; height:'.$shopSettings->y_small.'px;">'.$photo_out.'</a>
			</div>
			<br/>
			'.$rating_out.'
			<table border="0" width="100%" cellpadding="0" style="border-collapse: collapse" class="shop_item_cell_but" align="center" '.$click.' title="'.$shopSettings->sticker_add_to_cart.'">
				<tr>
					<td>
						<div class="section_item_but_left">'.$price_old_out.$price_out.'</div>
					</td>
					<td width="40">
						<div class="section_item_but_right">
							<img alt="'.$_item['title'].'" src="/components/shop/frontend/tmp/images/basket.png" class="section_item_cart_img"/>
						</div>
					</td>
				</tr>
			</table>
