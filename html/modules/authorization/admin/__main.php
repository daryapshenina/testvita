...
if ($act == "update")
{
	...
	$query_updatedit_modeule_quote = "UPDATE `modules` SET `title` = '$mod_title', `pub` = '$mod_pub', `titlepub` = '$mod_titlepub', `block` = '$mod_block', `ordering` = '$mod_ordering' WHERE `module` = 'authorization' LIMIT 1 ;";
			
	$sql_module_quote = mysql_query($query_updatedit_modeule_quote) or die ("Невозможно обновить данные");
	...
	global $site;
		...
		$num = mysql_query("SELECT * FROM `modules` WHERE `module` = 'authorization'") or die ("Невозможно сделать выборку из таблицы - 1");
		...
		echo '
			<div id="main-top"><img border="0" src="http://'.$site.'/modules/slider_skitter/admin/images/ico.png" width="25" height="25"  style="float: left; padding-top: 2px;" />&nbsp;&nbsp;Модуль "Авторизация"</div>

			<div class="padding-horizontal-20">

				<form method="POST" action="http://'.$site.'/admin/modules/authorization/update/">
