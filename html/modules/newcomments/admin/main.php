...
if ($act == "update")
{
	...
	$query_update_newcomments = "UPDATE `modules` SET `title` = '$mod_title', `pub` = '$mod_pub', `titlepub` = '$mod_titlepub', `p1` = '$quantity', `p2` = '$length', `block` = '$mod_block', `ordering` = '$mod_ordering' WHERE `module` = 'newcomments' LIMIT 1 ;";
			
	$sql_newcomments = mysql_query($query_update_newcomments) or die ("Невозможно обновить данные");
	...
	global $site, $quantity;
		...
		$num = mysql_query("SELECT * FROM `modules` WHERE `module` = 'newcomments'") or die ("Невозможно сделать выборку из таблицы - 1");
		...
		echo '
			<div id="main-top"><img border="0" src="http://'.$site.'/modules/newcomments/admin/images/ico.png" width="25" height="25"  style="float: left; padding-top: 2px;" />&nbsp;&nbsp;Модуль "Новые комментарии"</div>

			<div class="padding-horizontal-20">

				<form method="POST" action="http://'.$site.'/admin/modules/newcomments/update/">
				...
				<table class="main-tab">
					...
					<tr>
						<td width="200" height="25">Сколько выводить комментариев</td>
		...
		{
			echo '<div id="main-top">Модуль "newcomments" не подключён</div>';
		}
	} // конец функции
}
