...
if ($act == "update")
{
	...
	$query_update_newusers_quote = "UPDATE `modules` SET `title` = '$mod_title', `pub` = '$mod_pub', `titlepub` = '$mod_titlepub', `p1` = '$quantity', `block` = '$mod_block', `ordering` = '$mod_ordering' WHERE `module` = 'newusers' LIMIT 1 ;";
			
	$sql_newusers_quote = mysql_query($query_update_newusers_quote) or die ("Невозможно обновить данные");
	...
	global $site, $quantity;
		...
		$num = mysql_query("SELECT * FROM `modules` WHERE `module` = 'newusers'") or die ("Невозможно сделать выборку из таблицы - 1");
		...
		echo '
			<div id="main-top"><img border="0" src="http://'.$site.'/modules/newusers/admin/images/ico.png" width="25" height="25"  style="float: left; padding-top: 2px;" />&nbsp;&nbsp;Модуль "Новые участники"</div>

			<div class="padding-horizontal-20">

				<form method="POST" action="http://'.$site.'/admin/modules/newusers/update/">
				...
				<table class="main-tab">
					...
					<tr>
						<td width="200" height="25">Сколько выводить участников</td>
		...
		{
			echo '<div id="main-top">Модуль "newusers" не подключён</div>';
		}
	} // конец функции
}
